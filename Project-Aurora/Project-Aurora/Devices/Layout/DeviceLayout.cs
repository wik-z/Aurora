...
namespace Aurora.Devices.Layout
{
    ...
    public abstract class DeviceLayout : Settings.Settings, IDisposable
    {
        #region settings
        private System.Drawing.Point location = new System.Drawing.Point();
        [JsonIgnore]
        public System.Drawing.Point Location { get { return location; } set { var old = location; location = value; InvokePropertyChanged(old, value); } }

        private short selectedSDK = -1;
        [JsonIgnore]
        public short SelectedSDK { get { return selectedSDK; } set { var old = selectedSDK; selectedSDK = value; InvokePropertyChanged(old, value); } }

        private bool enabled = true;
        [JsonIgnore]
        public bool Enabled { get { return enabled; } set { var old = enabled;  enabled = value; InvokePropertyChanged(old, value); } }

        [JsonIgnore]
        public abstract VariableRegistry DefaultProperties { get; }

        public VariableRegistry Properties { get; protected set; } = new VariableRegistry();
        #endregion
        ...
        public DeviceLayout()
        {
            //Combine the saved properties with that of the default so it has the necessary updated properties if any have changed
            Properties.Combine(DefaultProperties, true);
        }
    ...
    public class MouseDeviceLayout : DeviceLayout
    {
        ...
        public override VariableRegistry DefaultProperties => new VariableRegistry( ("style", new VariableRegistryItem(PreferredMouse.Generic_Mouse, null, PreferredMouse.Generic_Mouse, "Style", "Mouse Style")) );
        ...
        public override void Initialize()
        {
            ...
            switch (this.Properties.GetVariable<PreferredMouse>("style"))
            {
        ...
        private Grid CreateUserControl(bool abstractKeycaps = false)
        {
            ...
            foreach (VirtualLight key in virtualGroup.grouped_keys)
            {
                ...
                if (key.tag != (LEDINT)MouseLights.NONE && !virtualKeyboardMap.ContainsKey(key.tag) && keycap is IKeycap && !abstractKeycaps)
                ...
                /*if (key.tag == (LEDINT)MouseLights.ESC)
                {
            ...
            //Update size
            new_virtual_keyboard.Width = virtualGroup.Region.Width;
            new_virtual_keyboard.Height = virtualGroup.Region.Height;
    ...
    public static class DeviceLayoutUtils
    {
        ...
        {
            return KeyboardDeviceLayout.GetDeviceLED((LEDINT)key);
        }

        public static DeviceLED GetDeviceLED(this MouseLights light)
        {
            return MouseDeviceLayout.GetDeviceLED((LEDINT)light);
        }
    }
    ...
    public class KeyboardDeviceLayout : DeviceLayout
    {
        ...
        public new const byte DeviceTypeID = 0;

        //With default/no PreferredKeyboard it will just use the clean language layout selected
        public override VariableRegistry DefaultProperties => new VariableRegistry( ("style", new VariableRegistryItem(PreferredKeyboard.None, null, PreferredKeyboard.None, "Style", "Keyboard Style")), 
                                                                                    ("language", new VariableRegistryItem(PreferredKeyboardLocalization.None, null, PreferredKeyboardLocalization.None, "Language", "Keyboard Language")) );
        ...
        public override void Initialize()
        {
            try
            {
                ...
                if (Directory.Exists(layoutsPath))
                {
                    ...
                    switch (this.Properties.GetVariable<PreferredKeyboardLocalization>("language"))
                    {
                ...
                PreferredKeyboard keyboard_preference = this.Properties.GetVariable<PreferredKeyboard>("style");
        ...
        private Grid CreateUserControl(bool abstractKeycaps = false)
        {
            ...
            foreach (VirtualLight key in virtualGroup.grouped_keys)
            {
                ...
                if (key.tag != (LEDINT)KeyboardKeys.NONE && !virtualKeyboardMap.ContainsKey(key.tag) && keycap is IKeycap && !abstractKeycaps)
                    virtualKeyboardMap.Add(key.tag, keycap as IKeycap);
                ...
                if (key.tag == (LEDINT)KeyboardKeys.ESC)
                {
                    baseline_x = keycap.Margin.Left;
                    baseline_y = keycap.Margin.Top;
                }
            ...
            if (virtualGroup.grouped_keys.Count == 0)
            {
    ...
    public class DeviceLayoutSettings
    {
        //DeviceType -> List of Devices of that type
        public Dictionary<byte, ObservableCollection<DeviceLayout>> Devices = new Dictionary<byte, ObservableCollection<DeviceLayout>>();
        
        public DeviceLayoutSettings()
        {
