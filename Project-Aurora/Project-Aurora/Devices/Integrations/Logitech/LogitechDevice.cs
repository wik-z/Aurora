...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
    ...
    class LogitechDevice : DeviceIntegration
    {
        ...
        public Dictionary<byte, byte> SupportedDeviceTypes => throw new NotImplementedException();
            ...
            try
            {
                foreach (DeviceLayout device in devices)
                {
                    ...
                    if (device.GetDeviceTypeID == KeyboardDeviceLayout.DeviceTypeID)
                    {
                        ...
                        {
                            if (token.IsCancellationRequested) return false;
                            Logitech_keyboardBitmapKeys localKey = ToLogitechBitmap(device.GetDeviceLED(key.Key));
                    ...
                    {
                        Color mouseColor;
                        if (device.DeviceColours.deviceColours.ContainsKey((short)MouseLights.Peripheral_Logo))
                            mouseColor = device.DeviceColours.deviceColours[(short)MouseLights.Peripheral_Logo];
                        else
                            mouseColor = device.DeviceColours.deviceColours.Values.First();

                        SendColorToPeripheral(mouseColor, forced || !peripheral_updated);
                    }
                }
                return true;
            }
        ...
        public static Logitech_keyboardBitmapKeys ToLogitechBitmap(DeviceLED key)
        {
            return ToLogitechBitmap(key.LedID, () => Global.deviceManager.GetDeviceFromDeviceLED(key).Properties.GetVariable<KeyboardDeviceLayout.PreferredKeyboardLocalization>("language"));
        }

        /// <summary>
        /// Uses func for the language so it isn't being computed every time a code is being looked up, only when it is needed
        /// </summary>
        /// <param name="key"></param>
        /// <param name="lang"></param>
        /// <returns></returns>
        public static Logitech_keyboardBitmapKeys ToLogitechBitmap(short key, Func<KeyboardDeviceLayout.PreferredKeyboardLocalization> lang)
        {

            switch ((KeyboardKeys)key)
            {
                ...
                case (KeyboardKeys.OEM5):
                    if (lang.Invoke() == KeyboardDeviceLayout.PreferredKeyboardLocalization.jpn)
